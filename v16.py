...
MIN_APPROACH_TIME_FOR_CLOSE_CAPTURE_SEC = 0.85
LOST_CAPTURE_CLOSE_DISTANCE_CM = 24.0
LOST_CAPTURE_CLOSE_BBOX_H_PX = 105
VISIBLE_CAPTURE_DISTANCE_CM = 17.0
VISIBLE_CAPTURE_BBOX_H_PX = 155
VISIBLE_CAPTURE_BOTTOM_RATIO = 0.88
...
class SimpleCubeMissionV16(Node):
    def __init__(self):
        super().__init__("simple_cube_mission_v16")
        ...
        print("[BOOT] SimpleCubeMission v16")
        print("[MODE] full standalone, compressed camera only")
        print("[FLOW] search -> stop -> backtrack 1s -> fine align -> approach")
        print("[FLOW] lost close -> forward 1s -> servo down; visible very close after driving -> servo down")
    ...
    def current_target_is_contact_close(self):
        ...
        close_by_bottom = bottom_y >= self.image_height * VISIBLE_CAPTURE_BOTTOM_RATIO and bbox_h >= 120
