...
class TrackingCubeV4(Node):
    def __init__(self):
        ...
        self.PATROL_SPEED = 0.20
        self.BACKWARD_SPEED = -0.14
        self.APPROACH_FAST_SPEED = 0.15
        self.APPROACH_SLOW_SPEED = 0.072

        self.SEARCH_ANG = 0.92
        self.ALIGN_MIN_ANG = 0.10
        self.ALIGN_MAX_ANG = 0.58
        self.TURN_MIN_ANG = 0.18
        self.TURN_MAX_ANG = 0.95
        self.RECOVER_ANG = 0.32
        ...
        self.APPROACH_ROTATE_ONLY_PX = 90
        self.REACQUIRE_PIXEL_TOL = 70
        self.CENTER_HOLD_FRAMES = 2
        self.CONFIRM_FRAMES = 3
        self.LOST_TARGET_TIMEOUT = 0.55
        ...
        self.MAX_RAW_JUMP_PX = 110.0
        self.SEARCH_CENTER_LOCK_RATIO = 0.18
        self.SEARCH_PRELOCK_FRAMES = 2
        self.CENTER_SCORE_GAIN = 260.0
        self.SAME_COLOR_BONUS = 900.0
        self.RED_SIGNATURE_RELAX = 0.92

        self.MIN_HOLES_FOR_RANGE = 3
        self.SLOW_HOLE_PITCH_PX = 15.0
        self.STOP_HOLE_PITCH_PX = 24.0
        self.STOP_BBOX_H_PX = 188
        ...
        print('[BOOT] TrackingCube_V4 started')
    ...
    def center_locked_for_search(self):
        if not self.target_visible or self.target_obs is None or self.image_width is None:
            return False
        center_tol = self.image_width * self.SEARCH_CENTER_LOCK_RATIO
    ...
    def search_ready_target(self):
        if not self.lockworthy_target():
            return False
        if not self.center_locked_for_search():
            return False
    ...
    def set_state(self, new_state, text=None):
        if self.state == new_state:
            return
        if new_state in ('SEARCH_SWEEP', 'PATROL_FORWARD', 'RETURN_TO_MIDPOINT', 'TURN_LEFT_90') and not self.target_recently_seen():
            self.tracked_color = None
        self.state = new_state
        self.state_enter_time = time.monotonic()
        if new_state == 'ALIGN_TARGET' and self.target_obs is not None:
            self.tracked_color = self.target_obs.color
        elif new_state == 'APPROACH_TARGET' and self.target_obs is not None:
            self.tracked_color = self.target_obs.color
    ...
    def build_red_mask(self, hsv, bgr):
        ...
        lower_red_2 = np.array([168, 80, 40])
    ...
    def color_signature(self, color, roi_bgr, roi_hsv, roi_mask):
        ...
        if color == 'blue':
            hue_ratio = float(np.mean((hue >= 108) & (hue <= 123) & (sat >= 125)))
            dom_rb = float(mean_b - mean_r)
            dom_gb = float(mean_b - mean_g)
            color_conf = 0.26 * float(mean_s) + 0.36 * dom_rb + 0.24 * dom_gb + 75.0 * hue_ratio
            ok = (
                hue_ratio >= 0.68
                and mean_s >= 150.0
                and mean_v >= 50.0
        ...
        hue_ratio = float(np.mean(((hue <= 12) | (hue >= 168)) & (sat >= 105)))
        dom_br = float(mean_r - mean_b)
        dom_gr = float(mean_r - mean_g)
        color_conf = 0.22 * float(mean_s) + 0.30 * dom_br + 0.22 * dom_gr + 60.0 * hue_ratio
        ok = (
            hue_ratio >= 0.56 * self.RED_SIGNATURE_RELAX
            and mean_s >= 115.0
            and mean_v >= 55.0
            and mean_r >= 85.0
            and dom_br >= 38.0
    ...
    def detect_best_cube(self, frame):
        ...
        elif self.tracked_color is not None and self.state in ('ALIGN_TARGET', 'APPROACH_TARGET'):
            candidate_colors = [self.tracked_color]
        else:
        ...
        for color in candidate_colors:
            ...
            for contour in contours:
                ...
                same_color_bonus = self.SAME_COLOR_BONUS if self.tracked_color == color else 0.0
                score = (
                    1.80 * area
                    + 420.0 * fill_ratio
                    + 340.0 * extent
    ...
    def handle_search_sweep(self):
        if self.search_ready_target():
            self.stop_robot_once()
            self.set_state('ALIGN_TARGET', f'前方锁定 {self.target_obs.color} 方块，开始对准')
            return
    ...
    def handle_align_target(self):
        ...
        angular = self.clamp_abs(-0.85 * error_norm, self.ALIGN_MIN_ANG, self.ALIGN_MAX_ANG)
    ...
    def handle_approach_target(self):
        ...
        if not self.target_visible or self.target_obs is None or self.image_width is None:
            if self.front_dist <= self.HARD_FRONT_STOP_DIST + 0.03:
                self.request_shutdown('近距离丢失目标，按前方距离停车')
                return
            if self.target_recently_seen():
                self.publish_cmd(0.0, self.last_target_dir * self.RECOVER_ANG)
                return
        ...
        if abs(error) > self.APPROACH_ROTATE_ONLY_PX:
            angular = self.clamp_abs(-0.95 * error_norm, self.ALIGN_MIN_ANG, self.ALIGN_MAX_ANG)
            self.publish_cmd(0.0, angular)
            return
        ...
        if abs(error) <= self.ALIGN_PIXEL_TOL:
            ...
            angular = self.clamp_abs(-0.72 * error_norm, 0.06, 0.28)
    ...
    def status_loop(self):
        ...
        if self.target_visible and self.target_obs is not None:
            print(
                ...
                f'lock={self.tracked_color or 'none'} ' 
                f'front={self.front_dist:.2f}'
            )
        else:
            print(f'[STATUS] {self.state} color=none lock={self.tracked_color or 'none'} front={self.front_dist:.2f}')
