...
class RedCubePatrol(Node):
    ...
    def handle_search_rotate(self):
        if self.target_seen():
            self.stop_robot_once()
            self.set_state('TRACK_TARGET', 'Target found during startup scan.')
            return
        ...
        if remaining <= self.SEARCH_DONE_TOL:
            self.stop_robot_once()
            self.set_state('PATROL_FORWARD', 'Startup scan finished. Enter patrol.')
            return
    ...
    def handle_track_target(self):
        ...
        # 目标暂时消失：先按上一次方向小范围找回，不再继续直冲
        if self.target_recently_seen():
            self.publish_cmd(0.0, self.last_target_dir * self.REACQUIRE_ANG)
            return

        self.stop_robot_once()
        self.set_state('SEARCH_ROTATE', 'Target lost. Restart search.')
