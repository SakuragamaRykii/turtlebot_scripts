...
class MarkerDetector:
    def __init__(self):
        ...
        for name in names:
            ...
            if hasattr(cv2.aruco, 'ArucoDetector'):
                ...
                self.detector_specs.append((name, detector, None))
            else:
                params = cv2.aruco.DetectorParameters_create()
                self.detector_specs.append((name, dictionary, params))
        print(f'[MARKER] loaded {len(self.detector_specs)} aruco/apriltag dictionaries')
    ...
    def detect(self, frame, target_id):
        ...
        for _name, detector_or_dict, params in self.detector_specs:
            if hasattr(cv2.aruco, 'ArucoDetector'):
...
class TrackingCubeV6(Node):
    def __init__(self):
        super().__init__('tracking_cube_v6')
        ...
        print('[BOOT] TrackingCube v6')
        print(f'[PARAM] cube_grab={CUBE_LIDAR_GRAB_CM:.1f}cm zone_stop={ZONE_STOP_DISTANCE_CM:.1f}cm')
        print(f'[SERVO PARAM] gpio=BCM{SERVO_GPIO_BCM} open={SERVO_OPEN_DUTY} close={SERVO_CLOSE_DUTY} down={SERVO_DOWN_DUTY} up={SERVO_UP_DUTY}')
    ...
    def image_callback(self, msg):
        ...
        if self.destination_marker_id is not None:
            marker = self.marker_detector.detect(frame, self.destination_marker_id)
            self.update_zone_marker(marker)
        else:
            self.clear_zone_marker()
    ...
    def handle_approach_cube(self):
        ...
        if not self.target_visible or self.target_obs is None:
            if self.lost_cube_close_should_grab() or self.cube_recently_seen():
                self.set_state('GRAB_FORWARD', 'cube disappeared, push forward before servo down')
                return
    ...
    def handle_grab_forward(self):
        ...
        if elapsed < GRAB_EXTRA_FORWARD_SEC:
            self.publish_cmd(GRAB_EXTRA_FORWARD_SPEED, 0.0)
            return
        self.stop_robot_once()
        self.set_state('SERVO_DOWN', 'lower servo to capture cube')
    ...
    def handle_search_zone(self):
        if self.zone_marker_visible and self.zone_marker is not None and self.zone_seen_frames >= 2:
            self.stop_robot_once()
            self.set_state('ALIGN_ZONE', f'found zone marker {self.destination_marker_id}')
            return
        self.publish_cmd(0.0, SEARCH_ANG)
    ...
    def handle_align_zone(self):
        ...
        if abs(err) <= ZONE_ALIGN_PIXEL_TOL:
            ...
            self.stop_robot_once()
            if self.centered_frames >= CENTER_HOLD_FRAMES:
                self.set_state('APPROACH_ZONE', f'centered zone marker {self.destination_marker_id}')
            return
    ...
    def handle_approach_zone(self):
        if not self.zone_marker_visible or self.zone_marker is None:
            self.stop_robot_once()
            self.set_state('SEARCH_ZONE', 'zone marker lost')
            return

        if self.front_dist <= ZONE_STOP_DISTANCE_M or self.zone_marker.bbox_h >= ZONE_STOP_BBOX_H_PX:
            self.stop_robot_once()
            self.set_state('SERVO_UP', 'arrived at destination')
            return
        ...
        if abs(err) > 130:
            self.stop_robot_once()
            self.set_state('ALIGN_ZONE', 'zone marker off center')
            return
    ...
    def handle_backup_after_drop(self):
        elapsed = time.monotonic() - self.state_enter_time
        if elapsed < BACKUP_AFTER_DROP_SEC:
    ...
    def handle_turn_after_drop(self):
        ...
        if abs(error) <= math.radians(4.0):
            self.stop_robot_once()
            self.set_state('SEARCH_CUBE', 'continue searching')
            return
        angular = self.clamp_abs(0.75 * error, 0.05 * SPEED_SCALE, 0.18 * SPEED_SCALE)
        self.publish_cmd(0.0, angular)
    ...
    def control_loop(self):
        ...
        if self.state == 'SEARCH_CUBE':
            self.handle_search_cube()
        elif self.state == 'ALIGN_CUBE':
            self.handle_align_cube()
        elif self.state == 'APPROACH_CUBE':
            self.handle_approach_cube()
        elif self.state == 'GRAB_FORWARD':
            self.handle_grab_forward()
        elif self.state == 'SERVO_DOWN':
            self.handle_servo_down()
        elif self.state == 'TURN_TO_ZONE':
            self.handle_turn_to_zone()
        elif self.state == 'SEARCH_ZONE':
            self.handle_search_zone()
        elif self.state == 'ALIGN_ZONE':
            self.handle_align_zone()
        elif self.state == 'APPROACH_ZONE':
            self.handle_approach_zone()
        elif self.state == 'SERVO_UP':
            self.handle_servo_up()
        elif self.state == 'BACKUP_AFTER_DROP':
            self.handle_backup_after_drop()
        elif self.state == 'TURN_AFTER_DROP':
            self.handle_turn_after_drop()
    ...
    def status_loop(self):
        ...
        if self.zone_marker_visible and self.zone_marker is not None:
            err = self.zone_error_pixels()
            marker_text = (
                f'marker={self.zone_marker.marker_id} err={err:.0f} '
                f'h={self.zone_marker.bbox_h:.0f}'
            )
