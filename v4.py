...
class TrackingCubeV4(Node):
    def __init__(self):
        ...
        self.SEARCH_ANG = 0.12
        self.SEARCH_TRACK_MIN_ANG = 0.04
        self.SEARCH_TRACK_MAX_ANG = 0.14
        self.ALIGN_MIN_ANG = 0.035
        self.ALIGN_MAX_ANG = 0.12
        self.RECOVER_ANG = 0.09

        self.APPROACH_FAST_SPEED = 0.060
        self.APPROACH_SLOW_SPEED = 0.028

        self.EMERGENCY_STOP_DIST = 0.10
        self.HARD_FRONT_STOP_DIST = 0.115

        self.ALIGN_PIXEL_TOL = 22
        self.APPROACH_ROTATE_ONLY_PX = 90
        self.REACQUIRE_PIXEL_TOL = 72
        self.CENTER_HOLD_FRAMES = 2
        self.CONFIRM_FRAMES = 2
        self.LOST_TARGET_TIMEOUT = 0.80
        self.SEARCH_ENTER_ALIGN_RATIO = 0.34

        self.MIN_CONTOUR_AREA = 220
        self.MIN_TRACK_AREA = 260
        self.SEARCH_LOCK_MIN_AREA = 420
        self.MIN_BBOX_W = 12
        self.MIN_BBOX_H = 12
        self.MAX_ASPECT_RATIO = 1.95
        self.MIN_ASPECT_RATIO = 0.52
        self.MIN_FILL_RATIO = 0.24
        self.MIN_EXTENT = 0.19
        self.MIN_SOLIDITY = 0.66
        self.MIN_BOTTOM_Y_RATIO = 0.42
        self.MAX_RAW_JUMP_PX = 110.0

        self.MIN_HOLES_FOR_RANGE = 3
        self.SLOW_HOLE_PITCH_PX = 14.0
        self.STOP_HOLE_PITCH_PX = 22.0
        self.STOP_BBOX_H_PX = 180
    ...
    def target_confirmed(self):
        return self.search_candidate_ready() and self.target_seen_frames >= self.CONFIRM_FRAMES
    ...
    def scan_callback(self, msg):
        ...
        good = [x for x in front_vals if math.isfinite(x) and x > 0.05]
    ...
    def image_callback(self, msg):
        ...
        if self.prev_raw_obs is not None and obs.color == self.prev_raw_obs.color:
            ...
            if raw_jump <= self.MAX_RAW_JUMP_PX and 0.28 <= area_ratio <= 3.6:
                stable = True
        ...
        self.last_target_bbox_h = float(obs.bbox_h)
    ...
    def build_red_mask(self, hsv, bgr):
        ...
        lower_red_2 = np.array([168, 95, 45])
    ...
    def color_signature(self, color, roi_bgr, roi_hsv, roi_mask):
        ...
        if color == 'blue':
            ...
            conf = 0.30 * mean_s + 0.34 * dom_rb + 0.22 * dom_gb + 65.0 * hue_ratio
            ok = (
                hue_ratio >= 0.55
                and mean_s >= 145.0
                and mean_v >= 55.0
        ...
        hue_ratio = float(np.mean(((hue <= 12) | (hue >= 168)) & (sat >= 115)))
        dom_br = float(mean_r - mean_b)
        dom_gr = float(mean_r - mean_g)
        conf = 0.26 * mean_s + 0.34 * dom_br + 0.22 * dom_gr + 62.0 * hue_ratio
        ok = (
            hue_ratio >= 0.58
            and mean_s >= 130.0
            and mean_v >= 55.0
    ...
    def detect_best_cube(self, frame):
        ...
        best = None
        for color in ('blue', 'red') if self.TARGET_COLOR == 'any' else (self.TARGET_COLOR,):
            ...
            for contour in contours:
                ...
                if w < self.MIN_BBOX_W or h < self.MIN_BBOX_H:
                    continue
                if self.image_height is not None and (y + h) < self.image_height * self.MIN_BOTTOM_Y_RATIO:
                    continue
                ...
                bottom_ratio = (y + h) / float(self.image_height) if self.image_height else 0.5
                center_bonus = 1.0 - abs((x + w / 2.0) - (self.image_width / 2.0)) / max(self.image_width / 2.0, 1.0)
                color_bonus = 18.0 if color == 'blue' else 0.0
    ...
    def handle_search_sweep(self):
        if self.target_visible and self.search_candidate_ready() and self.image_width is not None:
            error = self.target_error_pixels()
            half = self.image_width / 2.0
            error_norm = error / half
            align_gate_px = self.image_width * self.SEARCH_ENTER_ALIGN_RATIO

            if abs(error) <= align_gate_px and self.target_seen_frames >= self.CONFIRM_FRAMES:
                self.stop_robot_once()
                self.set_state('ALIGN_TARGET', f'发现 {self.target_obs.color} 方块，开始对准')
                return

            angular = self.clamp_abs(-0.45 * error_norm, self.SEARCH_TRACK_MIN_ANG, self.SEARCH_TRACK_MAX_ANG)
    ...
    def handle_align_target(self):
        ...
        angular = self.clamp_abs(-0.28 * error_norm, self.ALIGN_MIN_ANG, self.ALIGN_MAX_ANG)
        self.publish_cmd(0.0, angular)
    ...
    def handle_approach_target(self):
        ...
        if not self.target_visible or self.target_obs is None or self.image_width is None:
            if self.last_target_pitch >= self.STOP_HOLE_PITCH_PX * 0.85 or self.last_target_bbox_h >= self.STOP_BBOX_H_PX * 0.84 or self.front_dist <= self.HARD_FRONT_STOP_DIST + 0.02:
        ...
        if self.target_obs.hole_pitch >= self.SLOW_HOLE_PITCH_PX or abs(error) > self.ALIGN_PIXEL_TOL * 1.8:
        ...
        if abs(error) <= self.ALIGN_PIXEL_TOL:
            ...
            angular = self.clamp_abs(-0.22 * error_norm, 0.02, 0.08)
