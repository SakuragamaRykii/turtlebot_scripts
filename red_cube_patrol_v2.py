...
class RedCubePatrol(Node):
    def __init__(self):
        super().__init__('red_cube_patrol_v2')

        # =========================
        # 可调参数（真机保守版）
        # =========================
        self.LINEAR_SPEED = 0.05                 # 靠近目标/巡逻前进统一速度
        self.BACKWARD_SPEED = -0.04             # 返回中点倒车速度

        self.SEARCH_MAX_ANG = 0.18              # 搜索最大转速
        self.SEARCH_MIN_ANG = 0.09              # 搜索最小转速（接近结束时减速）
        self.ALIGN_MAX_ANG = 0.20               # 对准目标最大转速
        self.ALIGN_MIN_ANG = 0.05               # 对准目标最小转速
        self.TURN_MAX_ANG = 0.18                # 90度转弯最大转速
        self.TURN_MIN_ANG = 0.06                # 90度转弯最小转速

        self.WALL_STOP_DIST = 0.50              # 巡逻时离墙停止距离
        self.APPROACH_STOP_DIST = 0.18          # 靠近目标时前向安全停止距离（防顶方块）
        self.EMERGENCY_WALL_DIST = 0.15         # 极近紧急停止距离

        self.ALIGN_PIXEL_TOL = 18               # 对准允许误差（像素）
        self.REACQUIRE_PIXEL_TOL = 110          # 前进中偏差过大则回到对准状态
        self.RED_MIN_AREA = 700                 # 红色目标最小面积阈值
        self.LOST_TARGET_FRAMES = 2             # 丢失目标多少帧后停止
        self.CENTER_HOLD_FRAMES = 3             # 连续居中多少帧后再前进

        self.SEARCH_DONE_TOL = math.radians(3)  # 搜索完成容差
        self.TURN_YAW_TOL = math.radians(3)     # 90度转向完成容差
        self.MIDPOINT_TOL = 0.03                # 返回中点进度容差（米）

        self.CONTROL_DT = 0.05                  # 20Hz 控制，更及时
        self.STATUS_DT = 1.00
    ...
    def handle_search_rotate(self):
        ...
        # 越接近360度越慢，减少转过头
        angular = min(self.SEARCH_MAX_ANG, 0.7 * remaining)
    ...
    def handle_approach_target(self):
        # 安全优先：距离太近立即停止，避免顶着方块走
        if self.front_dist <= self.APPROACH_STOP_DIST:
            self.stop_robot()
            self.set_state('TARGET_LOST_STOP', 'Front object too close. Stop for pickup placeholder.')
            return

        if self.front_dist <= self.EMERGENCY_WALL_DIST:
            self.stop_robot()
            self.set_state('TARGET_LOST_STOP', 'Emergency front stop triggered.')
            return
        ...
        angular = 0.0
        if abs(error) > self.ALIGN_PIXEL_TOL:
            angular = self.clamp_abs(-0.22 * error_norm, 0.03, 0.12)
    ...
    def handle_return_to_midpoint(self):
        ...
        # 用“直线段投影进度”判断是否到中点，避免简单欧氏距离导致倒退过头
    ...
    def handle_turn_left_90(self):
        ...
        # 比原版更慢、更稳的比例转向，减少转过头
        angular = self.clamp_abs(0.9 * error, self.TURN_MIN_ANG, self.TURN_MAX_ANG)
        self.publish_cmd(0.0, angular)
