...
class RedCubePatrol(Node):
    def __init__(self):
        super().__init__('red_cube_patrol_v4')
        ...
        # 关键行为选项
        self.AUTO_EXIT_ON_APPROACH_STOP = True
        self.AUTO_EXIT_ON_EMERGENCY_STOP = True
        ...
        self.shutdown_sent_count = 0
        self.status_wait_last = None
    ...
    def stop_robot_reliable(self, repeat=10, delay=0.03):
    ...
    def handle_approach_target(self):
        if self.front_dist <= self.EMERGENCY_WALL_DIST:
            self.stop_robot_once()
            if self.AUTO_EXIT_ON_EMERGENCY_STOP:
                self.request_shutdown('Emergency front stop triggered. Program will exit.')
            else:
        ...
        if self.state_age() >= self.APPROACH_MIN_TIME and travel >= self.APPROACH_MIN_TRAVEL:
            if self.front_dist <= self.APPROACH_STOP_DIST:
                self.stop_robot_once()
                if self.AUTO_EXIT_ON_APPROACH_STOP:
                    self.request_shutdown('Approach stop triggered. Program will exit.')
                else:
    ...
    def control_loop(self):
        if self.shutdown_requested:
            self.stop_robot_once()
            self.shutdown_sent_count += 1
            if self.shutdown_sent_count >= 8:
    ...
    def status_loop(self):
        if not self.ready():
            current_wait = (self.has_scan, self.has_odom, self.has_image)
            if current_wait != self.status_wait_last:
                print(
                    f'[STATUS] waiting scan={self.has_scan} '
                    f'odom={self.has_odom} image={self.has_image}'
                )
                self.status_wait_last = current_wait
            return

        if self.shutdown_requested:
            print(f'[STATUS] stopping state={self.state} pos=[{self.local_x:.2f}, {self.local_y:.2f}]')
            return

        target_text = 'yes' if self.red_visible else 'no'
        area_text = f'{self.red_area:.0f}' if self.red_visible else '0'
        print(
