...
class SimpleCubeMissionV14(base.SimpleCubeMissionV13):
    def __init__(self):
        super().__init__()
        self.detect_error_count = 0
        self.last_detection_ok_time = time.monotonic()
        print("[BOOT] SimpleCubeMission v14")
        print("[FIX] center hit -> stop -> reverse turn 1s -> fine align")
        print("[FIX] detector exceptions are caught so camera callback cannot kill node")
    ...
    def handle_center_stop(self):
        ...
        if self.state_age() >= base.base.CENTER_STOP_SEC:
    ...
    def handle_fine_align(self):
        if self.front_dist <= base.base.EMERGENCY_STOP_M:
        ...
        if not self.target_visible or self.target_obs is None:
            if self.state_age() < FINE_ALIGN_LOST_RECOVER_SEC:
                ...
                return
            self.set_state("SEARCH", "target still lost after fine-align recovery")
            return
        ...
        if abs(err) <= base.base.FINE_ALIGN_PIXEL_TOL:
            self.center_seen_frames += 1
            self.stop_robot_once()
            if self.center_seen_frames >= base.base.FINE_ALIGN_HOLD_FRAMES:
        ...
        angular = self.clamp_abs(
            -0.18 * err_norm,
            base.base.FINE_ALIGN_MIN_ANG,
            base.base.FINE_ALIGN_MAX_ANG,
        )
    ...
    def status_loop(self):
        ...
        print(
            ...
            f"detect_errors={self.detect_error_count} image=compressed {self.image_width}x{self.image_height}"
        )


def main(args=None):
    rclpy.init(args=args)
    node = SimpleCubeMissionV14()
    try:
        rclpy.spin(node)
