...
EXTRA_FORWARD_AFTER_LOST_SEC = 1.6
APPROACH_LOST_GRACE_FRAMES = 5
BLUE_APPROACH_LOST_GRACE_FRAMES = 14

# Close capture is intentionally strict. It should prevent long pushing after
# contact, but not fire immediately after alignment.
MIN_APPROACH_TIME_FOR_CLOSE_CAPTURE_SEC = 1.20
LOST_CAPTURE_CLOSE_DISTANCE_CM = 24.0
LOST_CAPTURE_CLOSE_BBOX_H_PX = 105
VISIBLE_CAPTURE_DISTANCE_CM = 13.0
VISIBLE_CAPTURE_BBOX_H_PX = 190
VISIBLE_CAPTURE_BOTTOM_RATIO = 0.94
...
class SimpleCubeMissionV17(Node):
    def __init__(self):
        super().__init__("simple_cube_mission_v17")
        ...
        print("[BOOT] SimpleCubeMission v17")
        print("[MODE] full standalone, compressed camera only")
        print("[FLOW] search -> stop -> backtrack 1s -> fine align -> approach")
        print("[FLOW] approach is faster; lost close -> forward 1.6s -> servo down")
    ...
    def current_target_is_contact_close(self):
        if not self.target_visible or self.target_obs is None or self.image_height is None:
            return False
        if self.state_age() < MIN_APPROACH_TIME_FOR_CLOSE_CAPTURE_SEC:
            return False
        ...
        close_by_distance = 0.1 < distance_cm <= VISIBLE_CAPTURE_DISTANCE_CM
        close_by_size = bbox_h >= VISIBLE_CAPTURE_BBOX_H_PX
        close_by_bottom = bottom_y >= self.image_height * VISIBLE_CAPTURE_BOTTOM_RATIO and bbox_h >= 150
    ...
    def handle_approach(self):
        ...
        if self.current_target_is_contact_close():
            self.stop_robot_once()
            self.set_state("SERVO_DOWN", f"visible contact close, {self.last_target_summary()}")
            return
