...
class CubeFinderV2(Node):
    def __init__(self):
        ...
        self.SEARCH_ANG = 0.39
        self.SEARCH_TRACK_MAX_ANG = 0.46
        self.ALIGN_MIN_ANG = 0.06
        self.ALIGN_MAX_ANG = 0.28
        self.RECOVER_ANG = 0.21
        self.APPROACH_FAST = 0.08
        self.APPROACH_SLOW = 0.05

        # Vision gates.
        self.MIN_TRACK_AREA = 260.0
        self.MIN_LOCK_AREA = 460.0
        self.MIN_BBOX = 16
        self.MIN_CENTER_Y_RATIO = 0.25
        self.MIN_FILL_RATIO = 0.24
        self.MIN_EXTENT = 0.20
        self.MIN_SOLIDITY = 0.72
        self.MIN_TRACK_BOTTOM_RATIO = 0.38
        self.MIN_LOCK_BOTTOM_RATIO = 0.48
        self.MAX_ASPECT = 1.95
        self.MIN_ASPECT = 0.52
        self.MAX_RAW_JUMP_PX = 120.0
        self.CONFIRM_FRAMES = 3
        self.CENTER_LOCK_PX = 95
        self.SEARCH_CENTER_HOLD_PX = 14
    ...
    def target_lockworthy(self):
        if not self.target_visible or self.target_obs is None or self.image_width is None or self.image_height is None:
            return False
        obs = self.target_obs
        err = abs(obs.cx - self.image_width / 2.0)
        bottom_ratio = (obs.bbox_y + obs.bbox_h) / float(self.image_height)
        min_conf = 95.0 if obs.color == 'blue' else 80.0
        return (
            obs.area >= self.MIN_LOCK_AREA
            and err <= self.CENTER_LOCK_PX
            and bottom_ratio >= self.MIN_LOCK_BOTTOM_RATIO
            and obs.fill_ratio >= self.MIN_FILL_RATIO
            and obs.extent >= self.MIN_EXTENT
            and obs.solidity >= self.MIN_SOLIDITY
            and obs.color_conf >= min_conf
        )
    ...
    def build_blue_mask(self, hsv, bgr):
        lower_blue = np.array([100, 95, 35], dtype=np.uint8)
    ...
    def color_signature(self, color, roi_bgr, roi_hsv, roi_mask):
        ...
        if color == 'blue':
            ...
            conf = 0.30 * float(mean_s) + 0.34 * dom_br + 0.22 * dom_bg + 65.0 * hue_ratio
            ok = (
                hue_ratio >= 0.55
                and mean_s >= 145.0
                and mean_v >= 55.0
    ...
    def detect_best_for_color(self, color, color_mask, frame, hsv, gray):
        ...
        for contour in contours:
            ...
            if self.image_height is not None:
                if cy < self.image_height * self.MIN_CENTER_Y_RATIO:
                    continue
                if bottom < self.image_height * self.MIN_TRACK_BOTTOM_RATIO:
                    continue
    ...
    def detect_best_cube(self, frame):
        ...
        if blue_best is not None and red_best is not None:
            if blue_best.score >= red_best.score * 0.72:
                return blue_best
            return red_best
    ...
    def handle_search(self):
        ...
        if self.target_visible and self.target_obs is not None and self.image_width is not None:
            ...
            if self.target_lockworthy():
                if abs(err) <= self.SEARCH_CENTER_HOLD_PX:
                    self.stop_robot_once()
                    return
                ang = self.clamp_abs(-0.0030 * err, 0.08, self.SEARCH_TRACK_MAX_ANG)
                self.publish_cmd(0.0, ang)
                return

            if abs(err) <= self.CENTER_LOCK_PX * 1.2:
                ang = -0.0024 * err
                if abs(err) <= 10.0:
                    self.stop_robot_once()
                else:
                    self.publish_cmd(0.0, self.clamp_abs(ang, 0.05, self.SEARCH_TRACK_MAX_ANG * 0.70))
                return
    ...
    def status_loop(self):
        ...
        if self.target_visible and self.target_obs is not None and self.image_width is not None:
            ...
            if self.image_height:
                bottom_ratio = (self.target_obs.bbox_y + self.target_obs.bbox_h) / float(self.image_height)
            print(
                f'[STATUS] {self.state} '
