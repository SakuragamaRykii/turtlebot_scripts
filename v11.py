...
PREFER_BLUE = True            # only matters when TARGET_COLOR == "any"

SEARCH_ANG = 0.16             # slow spin
SEARCH_CENTER_BAND_PX = 95    # if target box center is within this band, stop spinning and go forward
MIN_CENTER_FRAMES = 2         # stable centered frames before approach

APPROACH_SPEED = 0.045
EXTRA_FORWARD_SPEED = 0.040
EXTRA_FORWARD_AFTER_LOST_SEC = 1.5

TURN_AFTER_GRAB_MAX_ANG = 0.14
TURN_RETURN_MAX_ANG = 0.20
TURN_180_TOL_DEG = 4.0

FORWARD_AFTER_TURN_SPEED = 0.070
...
# If boot "up" moves down on your robot, swap these two values.
SERVO_UP_DUTY = 9.0
SERVO_DOWN_DUTY = 5.5
...
# =========================
# Detection parameters copied from CubeVision_Debug_v2 (2).py
# =========================
...
class SimpleCubeMission(Node):
    def __init__(self):
        super().__init__("simple_cube_mission_v11")
        ...
        print("[BOOT] SimpleCubeMission v11")
        print("[MODE] search -> center-band lock -> straight approach -> lost => forward 1.5s -> servo sequence")
    ...
    def raw_image_callback(self, msg):
        ...
        try:
            ...
            return
        self.process_frame(frame, msg.encoding.lower())
    ...
    def process_frame(self, frame, source):
        now = time.monotonic()
        if source == "compressed" and self.image_source not in ("none", "compressed") and now - self.last_image_time < 0.30:
            return
        ...
        if not self.image_logged:
            print(f"[IMAGE] source={source} size={self.image_width}x{self.image_height}")
    ...
    def status_loop(self):
        ...
        print(
            ...
            f"image={self.image_source} {self.image_width}x{self.image_height}"
        )
