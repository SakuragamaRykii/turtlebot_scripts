...
# v100: competition mode based on tested v42.
SEARCH_ANG = 0.176
...
class CubeDetector:
    ...
    def detect(self, frame, target_color=DEFAULT_TARGET_COLOR):
        ...
        if red_best is not None and blue_best is not None:
            if PREFER_BLUE:
                chosen = blue_best if blue_best.score >= red_best.score else red_best
            else:
                chosen = blue_best if blue_best.score > red_best.score else red_best
        else:
...
class SimpleCubeMissionV100(Node):
    def __init__(self):
        super().__init__("simple_cube_mission_v100")
        ...
        print("[BOOT] SimpleCubeMission v100 competition")
        print("[MODE] full standalone, compressed camera only")
        print("[FLOW] competition: grab red or blue cube, deliver all cubes to assigned target half")
        print("[FLOW] fast lock: search -> short stop -> fine align -> approach with steering -> lost -> forward 1.2s -> arm down")
        print("[FLOW] after arm down: backup 0.7s before turning, to keep cube in claw")
    ...
    def target_zone_marker_ids(self):
        if COMPETITION_TARGET_ZONE == "BLUE_ZONE":
    ...
    def delivery_ids_for_color(self, color):
        return self.target_zone_marker_ids()
    ...
    def handle_backup_after_release(self):
        ...
        self.servo.servo_up()
        self.current_zone = COMPETITION_TARGET_ZONE
        self.last_zone_reason = "released_in_competition_target_zone"
        if STOP_AFTER_MAX_DELIVERIES and self.completed_cycles >= MAX_COMPETITION_DELIVERIES:
            self.set_state("STOPPED", f"competition deliveries complete={self.completed_cycles}")
            return
    ...
    def status_loop(self):
        ...
        print(
            ...
            f"zone={self.current_zone} target_zone={COMPETITION_TARGET_ZONE} want={self.current_target_color} lock={self.locked_search_color} "
