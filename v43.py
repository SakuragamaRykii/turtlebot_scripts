...
# v43: exam-room mode. Faster lock, straight close approach.
SEARCH_ANG = 0.176
...
FINE_ALIGN_MAX_SEC = 3.0
FINE_ALIGN_TIMEOUT_PIXEL_TOL = 70

APPROACH_SPEED = 0.078
APPROACH_STEER_GAIN = 0.0010
APPROACH_STEER_MAX = 0.055
APPROACH_STRAIGHT_DISTANCE_CM = 34.0
APPROACH_STRAIGHT_BBOX_H = 72
APPROACH_STRAIGHT_BOTTOM_RATIO = 0.62
EXTRA_FORWARD_SPEED = 0.060
EXTRA_FORWARD_AFTER_LOST_SEC = 1.2
BACKUP_AFTER_GRAB_SPEED = -0.12
BACKUP_AFTER_GRAB_SEC = 1.05
...
class SimpleCubeMissionV43(Node):
    def __init__(self):
        super().__init__("simple_cube_mission_v43")
        ...
        self.approach_straight_mode = False
        ...
        print("[BOOT] SimpleCubeMission v43")
        ...
        print("[FLOW] close approach locks straight; after arm down backup 1.05s before turning")
        print("[FLOW] delivery drives with marker steering until lost/too-close/15s, then release")
        print("[FLOW] after release: backup 1s -> marker-width return center -> search")
    ...
    def target_is_close_for_straight_approach(self):
        if not self.target_visible or self.target_obs is None:
            return False
        distance_cm = self.target_obs.distance_cm
        bottom_y = self.target_obs.bbox_y + self.target_obs.bbox_h
        close_by_distance = 0.1 < distance_cm <= APPROACH_STRAIGHT_DISTANCE_CM
        close_by_size = self.target_obs.bbox_h >= APPROACH_STRAIGHT_BBOX_H
        close_by_bottom = (
            self.image_height is not None
            and bottom_y >= self.image_height * APPROACH_STRAIGHT_BOTTOM_RATIO
        )
    ...
    def handle_fine_align(self):
        ...
        if self.state_age() >= FINE_ALIGN_MAX_SEC and abs(err) <= FINE_ALIGN_TIMEOUT_PIXEL_TOL:
            self.grab_color = self.target_obs.color
            self.remember_target_for_capture()
            self.set_state("APPROACH", f"{self.grab_color} timeout accept err={err:.0f}, approach")
            return
    ...
    def handle_approach(self):
        ...
        self.approach_lost_frames = 0
        self.remember_target_for_capture()
        if self.target_is_close_for_straight_approach():
            self.approach_straight_mode = True

        err = self.target_error_pixels()
        if self.approach_straight_mode:
            steer = 0.0
        else:
    ...
    def handle_servo_down(self):
        ...
        self.set_state("BACKUP_AFTER_GRAB", f"backup 1.05s before turn, marker ids={sorted(self.delivery_marker_ids)}")
